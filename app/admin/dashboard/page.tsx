...
export default function DashboardPage() {
  ...
  const handleGenerateNoPeserta = async () => {
    ...
    if (confirm(`Generate No Peserta otomatis untuk ${students.length} siswa dengan awalan ${kodeSekolah}?`)) {
      ...
      } catch (err: any) { alert("Gagal update No Peserta: " + err.message); }
    }
  }

  // --- LOGIK IMPORT & TEMPLATE ---
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '16px', width: '400px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <label style={{ backgroundColor: '#1e293b', color: 'white', padding: '12px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>
                📁 Pilih File & Import
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '16px', width: '350px' }}>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <input placeholder="Masukkan Kode (misal: 050658)" value={kodeSekolah} onChange={(e) => setKodeSekolah(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1' }} />
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowModalGenNoPes(false)} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: 'none', backgroundColor: '#f1f5f9' }}>Batal</button>
              <button onClick={handleGenerateNoPeserta} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: 'none', backgroundColor: '#8b5cf6', color: 'white', fontWeight: 'bold' }}>Generate</button>
            </div>
          </div>
        </div>
      )}
